 ...
 */
/*
 *  $Id: inode.c,v 1.243 2005/11/20 23:03:34 arunmk Exp $
 */
...
extern struct dentry *unionfs_lookup(struct inode *, struct dentry *,
				     struct nameidata *);
extern int unionfs_readlink(struct dentry *dentry, char *buf, int bufsiz);
			...
			struct dentry *new_dentry)
{
	...
	name = KMALLOC(new_dentry->d_name.len + WHLEN + 1, GFP_UNIONFS);
	if (!name) {
		err = -ENOMEM;
		goto out;
	}
	...
	name[WHLEN + new_dentry->d_name.len] = '\0';
...
int unionfs_readlink(struct dentry *dentry, char *buf, int bufsiz)
{
	int err;
